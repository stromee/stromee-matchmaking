		...
		persist(
			(set) => ({
				...
				updateSelection: (selection) => {
					...
					set((state) => {
						const newRemaining = selection.filter(
							(item) =>
								!state.swiped.some((id) => id === item.id),
						);
				...
				onSwipe: ({ direction, id }) => {
					set((state) => {
						const index = state.remaining.findIndex(
							(item) => item.id === id,
						);
						if (index === -1) {
				...
				onSwipeFinished: ({ id }) => {
					set((state) => {
						...
						const newRemainingDeferred =
							state.remainingDeferred.filter(
								(item) => item.id !== id,
							);
